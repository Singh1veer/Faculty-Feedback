...
function FacultyProfile() {
  ...
  if (notFound) {
    return (
      <div className={shellClass}>
        <div className="text-center">
          <p className="font-mono text-[11px] uppercase tracking-[0.25em] text-oxblood">Record not found</p>
          <p className="font-display italic text-2xl text-ink mt-2">No entry for "{name}"</p>
          <Link to="/faculty" className="inline-block mt-6 font-mono text-xs uppercase tracking-wider text-ivy border-b border-ivy/40 hover:border-ivy pb-0.5 transition-colors duration-150">
            ← Back to directory
          </Link>
        </div>
      </div>
    ...
    <div className={shellClass}>
      <div className="w-full max-w-md">
        <div className="flex items-center justify-between mb-6">
          <Link to="/faculty" className="font-mono text-xs uppercase tracking-wider text-ivy border-b border-ivy/40 hover:border-ivy pb-0.5 transition-colors duration-150">
            ← Directory
          </Link>
          {isSignedIn && (
        ...
        <div className="relative rounded-sm border border-rule bg-paper-raised px-8 pt-9 pb-8 shadow-[0_1px_0_var(--color-rule)] animate-[fadeIn_0.3s_ease]">
          <div className="absolute -top-3 left-8 bg-brass text-paper-raised font-mono text-[10px] uppercase tracking-[0.2em] px-3 py-1 rounded-sm">
            Faculty Record
          </div>

          <h1 className="font-display text-4xl text-ink mt-2">{faculty.name}</h1>
          <p className="font-mono text-xs uppercase tracking-widest text-ivy mt-2">{faculty.department}</p>
        ...
        <Link
          to={`/faculty/${name}/comments`}
          className="inline-block mt-6 font-mono text-xs uppercase tracking-wider text-ivy border-b border-ivy/40 hover:border-ivy pb-0.5 transition-colors duration-150"
        >
          View & write comments →
        </Link>
      </div>
